bohist <- function(fig_height = NULL, fig_width = NULL, fig_title = NULL,
                   x_lab = NULL, y_lab = NULL, xlim_min = NULL, 
                   xlim_max = NULL, ylim_min = NULL, ylim_max = NULL,
                   x_grid = TRUE, y_grid = TRUE, x_axes = 'below', 
                   y_axes = 'left', legend_loc = 'top_right',
                   x_data = NULL, data = NULL, h_breaks = 'Sturges',
                   h_freq = TRUE, h_incl_low = TRUE, h_right = TRUE,
                   h_fill_col = 'blue', h_fill_alpha = 1, h_line_col = 'black',
                   h_line_alpha = 1, h_line_width = 1, add_density = FALSE,
                   den_col = 'black', den_alpha = 1, den_width = 1, 
                   den_type = 1, den_leg = FALSE, add_text = FALSE, text_x = NULL,
                   ...
                   min_bor_right = 50, min_bor_top = 50, bg_fill_alpha = 0.95, bg_fill_color = "#444444",
  ...
  h <- figure(height = fig_height, width = fig_width, title = fig_title,
              xlim = c(xlim_min, xlim_max), ylim = c(ylim_min, ylim_max),
              xlab = x_lab, ylab = y_lab, x_grid = x_grid, y_grid = y_grid,
              x_axes = x_axes, y_axes = y_axes, legend_loc = legend_loc) %>%
    ly_hist(x = xdata, data = data, breaks = h_breaks, 
            freq = h_freq, include.lowest = h_incl_low, right = h_right,
            fill_color = h_fill_col, fill_alpha = h_fill_alpha, 
            line_color = h_line_col, line_alpha = h_line_alpha, 
...
hi <- bohist(x_data = 'eruptions', data = faithful, h_breaks = 5)
hi
